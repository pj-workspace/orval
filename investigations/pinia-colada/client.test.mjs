...
test('emitted key and request resolve the current getter and preserve AbortSignal', async () => {
  ...
  let id = 1;
  const optionsGetter = context.useGetPet(() => id);
  const firstOptions = optionsGetter();
  assert.equal(JSON.stringify(optionsGetter().key), '["getPet",1]');
  id = 2;
  assert.equal(JSON.stringify(optionsGetter().key), '["getPet",2]');
